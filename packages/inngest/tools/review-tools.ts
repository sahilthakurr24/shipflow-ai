  ...
  return createTool({
    ...
    parameters: z.object({
      ...
      title: z
        .string()
        .max(300)
        .describe("A one-line summary specific enough to act on without reading the description"),
      description: z
        .string()
        .min(1)
...
export function createSubmitVerdictTool(reviewId: string): Tool.Any {
  return createTool({
    name: "submit_verdict",
    description:
      "Submit the final verdict for this review. Call exactly once, after reviewing every changed file.",
    parameters: z.object({
      ...
      readinessScore: z
        ...
        .min(0)
        .max(100)
        .describe("Honest estimate of how ready this is to ship as-is"),
    }),
